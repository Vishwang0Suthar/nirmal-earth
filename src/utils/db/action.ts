  ...
  try {
    ...
    await Promise.all([
      ...
      createTransaction(
        ...
        "earned_report",
        pointsEarned,
        "Earned points for reporting waste"
      ),
      createNotification(
        user_id,
...
export async function getReportsByuser_id(user_id: number) {
  try {
    const reports = await db
      .select()
      .from(Reports)
      .where(eq(Reports.user_id, user_id))
      .execute();
    return reports;
  } catch (error) {
...
export async function getUnreadNotifications(user_id: number) {
  try {
    return await db
      ...
      .from(Notifications)
      .where(
        and(eq(Notifications.user_id, user_id), eq(Notifications.isRead, false))
      )
      .execute();
  } catch (error) {
  ...
  try {
    const rewards = await db
      .select({
        ...
        userName: Users.name,
        totalPoints: sql<number>`SUM(${Rewards.points})`.as("totalPoints"),
        maxLevel: sql<number>`MAX(${Rewards.level})`.as("maxLevel"),
      })
      .from(Rewards)
      .leftJoin(Users, eq(Rewards.user_id, Users.id))
      .groupBy(Rewards.user_id, Users.name) // Grouping by user
      .orderBy(desc(sql`SUM(${Rewards.points})`)) // Order by total points
      .execute();
