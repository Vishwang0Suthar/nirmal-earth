...
export async function createReport(
  ...
  try {
    const [report] = await db.insert(Reports).values({
      ...
      status: "pending",
    }).returning().execute(); // Adjust as needed
    ...
    await Promise.all([
      updateRewardPoints(user_id, pointsEarned),
      createTransaction(user_id, "earned_report", pointsEarned, "Earned points for reporting waste"),
      createNotification(user_id, "Report submitted successfully", "report_submitted"),
    ]);

    return report;
  } catch (e) {
    console.error("Error creating report:", e);
    throw new Error("Internal server error while creating the report.");
  }
}


export async function getReportsByuser_id(user_id: number) {
  try {
    const reports = await db.select().from(Reports).where(eq(Reports.user_id, user_id)).execute();
    return reports;
  } catch (error) {
...
export async function getOrCreateReward(user_id: number) {
  try {
    let [reward] = await db.select().from(Rewards).where(eq(Rewards.user_id, user_id)).execute();
    if (!reward) {
...
export async function updateRewardPoints(user_id: number, pointsToAdd: number) {
  try {
...
export async function createNotification(user_id: number, message: string, type: string) {
  try {
...
export async function getUnreadNotifications(user_id: number) {
  try {
...
export async function saveReward(user_id: number, amount: number) {
  try {
    ...
    await createTransaction(user_id, 'earned_collect', amount, 'Points earned for collecting waste');
...
export async function getRewardTransactions(user_id: number) {
  try {
...
export async function getAvailableRewards(user_id: number) {
  try {
    ...
    const userTransactions = await getRewardTransactions(user_id);
...
export async function createTransaction(user_id: number, type: 'earned_report' | 'earned_collect' | 'redeemed', amount: number, description: string) {
  try {
...
export async function redeemReward(user_id: number, rewardId: number) {
  try {
    ...
    if (rewardId === 0) {
      ...
      await createTransaction(user_id, 'redeemed', userReward.points, `Redeemed all points: ${userReward.points}`);
      ...
      // Create a transaction for this redemption
      await createTransaction(user_id, 'redeemed', availableReward[0].points, `Redeemed: ${availableReward[0].name}`);
